myApp.controller('FAttack', function ($scope, $rootScope, $location) {

  $scope.init = function () {
    console.log("init Fattack");
    $scope.result = 0;
    //$scope.result = 0 if not decided , 1 if correct , 2 if faulty
  };
  ...
  var X0 = 0, Y0 = 0, Z0 = 0,
      X1 = 0, Y1 = 0, Z1 = 0;
  var okAx = 0, okAy = 0, okAz = 0;// okAn = 0 daca inca nu s-a sesizat o miscare importanta
  //      = 1 sau -1 daca miscarea s-a realizat pe semiaxa pozitiva, respectiv negativa n.
  var readyToConclude = 0;
  ...
  function onDeviceReady() {
    $scope.practice = function () {
      ...
      setTimeout(function () { readyToConclude = 1; }, 2500);
    }
    //nerelevant::
  ...
  function avrageToVelocity(inputArray, dT) {//vectorul de input pt medie si delta T in secunde
            ...
            if (inputArray[i][3] <= avgPlusS.z) {
              if (inputArray[i][2] >= avgMinusS.y) {
                if (inputArray[i][2] <= avgPlusS.y) {
                  ...
                  if (readyToConclude == 0) {
                    if (inputArray[i][1] < -1.5) {
                      if (okAx == 0) {
                        okAx = -1;
                      }
                    }
                    else if (inputArray[i][1] > 1.5) {
                      if (okAx == 0) {
                        okAx = 1;
                      }
                    }
                    if (inputArray[i][2] < -1.5) {
                      if (okAy == 0) {
                        okAy = -1;
                      }
                    }
                    else if (inputArray[i][2] > 1.5) {
                      if (okAy == 0) {
                        okAy = 1;
                      }
                    }
                    if (inputArray[i][3] < -1.5) {
                      if (okAz == 0) {
                        okAz = -1;
                      }
                    }
                    else if (inputArray[i][3] > 1.5) {
                      if (okAz == 0) {
                        okAz = 1;
                      }
                    }
    ...
    if (readyToConclude == 1) {
      console.log("verdictul");
      //var message = 'conclusion:<br/> okAx=' + okAx + '<br /> okAy=' + okAy + '<br /> okAz=' + okAz;
