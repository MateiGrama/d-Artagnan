myApp.controller('ProbaFencing1', function ($scope, $rootScope, $location) {

  $scope.init = function () {
    console.log("init probaFencing1");
    $scope.result = 0;
  ...
  function onDeviceReady() {
    ...
    function acctualXlr8(acceleration, rotation) {
      ...
      var gamma = (rotation.beta+180) * Math.PI / 180;
      //A SE MODIFICA IN FUNCTIE DE CE SE OBSERVA
      when(
        function (done) {
    ...
    function avrageToVelocity(inputArray, dT) {//vectorul de input pt medie si delta T in secunde
      ...
      //Interpretarea miscarii:
      var distFrom0=Math.sqrt(X1*X1+Y1*Y1+Z1*Z1);
      if (distFrom0 > 1) {
        $scope.result = 1;
        $scope.$apply();
      }
